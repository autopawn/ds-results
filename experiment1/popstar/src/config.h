...
class PMConfig {
	public:
		...
		double mintime;     //mininum time for grasp testing: algorithm will be rerun until this time
		...
		bool output;        //output file with solution?
		...
		bool test_speedup;  //test speedup
		...
		double motime;   //minimum oracle time
...
bool PMConfig::readParameters (int _argc, char *_argv[]) {
	...
	while (arg < argc) {
		...
		if (readDouble ("-motime",    motime)) continue;
		if (readDouble ("-mintime",   mintime)) continue;
		if (readBool   ("-test_speedup",   test_speedup)) continue;
