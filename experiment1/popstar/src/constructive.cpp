...
void PMConstructive::setMethod (char *m) {
	...
	if (tryMethod (m, "indep", INDEP, 1)) return;
...
void PMConstructive::run (PMSolution *s, int k) {
	...
	switch (m) {
		...
		case INDEP   : independent (s, v, false); break;
...
void PMConstructive::addRandom (PMSolution *s, int k) {
	int i;
	//fprintf (stderr, "Adding random %d.\n", k);
	...
	/*
	fprintf (stderr, "Random added (%f).\n", s->calcCost());
	for (i=1; i<=s->getN(); i++) {
		fprintf (stderr, "%d ", s->getClosest(i));
	}*/
}
...
/*---------------------------------------------------------
 | Constructive algorithm for the independent set problem. 
 | In the greedy version, always opens the set that has
 | the lowest degree in the residual graph.
 *-----------------------------------------------------*/
void PMConstructive::independent (PMSolution *s, double alpha, bool stats) {
	//get instance and related data

	if (!s->getInstance()->isIndep()) {
		fatal ("independent", "function can only be applied to independent set instances");
	}
	PMIndepInstance *inst = (PMIndepInstance*) s->getInstance();
	int nvertices = inst->getNVertices();
	//int nedges    = inst->getNEdges();
	
	//initialize auxiliary variables
	int v, w;
	int selcount = 0;
	int *candlist = new int [nvertices+1];      //will hold list of candidates in each round
	double *canddeg = new double [nvertices+1]; //canddeg[i] is the degree of candlist[i]
	bool *candidates = new bool [nvertices+1];  //vertices that are still candidates (all at first)
	int *rdegree = new int [nvertices+1];       //residual degree of the candidates
	bool *selected = new bool[nvertices+1];     //vertices in the independent set (none at first)


	for (v=1; v<=nvertices; v++) {
		candidates[v] = true;            //v starts as a candidate
		rdegree[v] = inst->getDegree(v); //residual degree is just original degree
		selected[v] = false;             //nobody belongs to the solution yet
	}
	...
	for (;;) {
		...
		int candcount = 0; //total number of candidates left
		for (v=1; v<=nvertices; v++) {
			if (candidates[v]) { //found a candidate -> add to list
				candcount++;
				candlist[candcount] = v;
				canddeg[candcount] = rdegree[v];
			}
		}
		if (candcount == 0) break;

		//select smallest vertex
		int restricted_size = (int) ceil (alpha * (double)candcount);
		if (restricted_size < 1) restricted_size = 1;
		i = RFW::randomSmallest(canddeg, 1, candcount, restricted_size);
		v = candlist[i];
		//fprintf (stderr, "Selected vertex %d.\n", v);

		//vertex v will belong to the solution
		if (selected[v]) fatal ("independent", "bug");
		candidates[v] = false;
		selected[v] = true;
		selcount++;
		rdegree[v] = 0;

		//make sure its neighbors don't
		int *neighborsv = inst->getNeighborList(v);
		for (i=inst->getDegree(v)-1; i>=0; i--) {
			//v's neighbor (w) will not belong to the solution
			w = neighborsv[i];
			if (candidates[w]) {
				candidates[w] = false;
				selected[w] = false;
				...
				//decrease the residual degree of w's neighbors
				int *neighborsw = inst->getNeighborList(w);
				for (int j=inst->getDegree(w)-1; j>=0; j--) {
					int x = neighborsw[j];
					if (candidates[x]) rdegree[x]--;
				}
			}
		}	
	}
	...
	fprintf (stderr, "%d vertices where selected.\n", selcount);
}
...
void PMConstructive::greedy (PMSolution *s, int k, bool proportional, double alpha, int *initlist) {
	...
	while (s->getOpenCount() < k) {
		...
		//fprintf (stderr, "s%f ", savelist[bestj]);
