...
double PMRelink::combinePR (PMSolution *t, PMSolution *s1, PMSolution *s2) {
	...
	t->copyFrom (s1);
	sm->search (t, true, single_elite, true); //perform path-relinking
...
double PMRelink::combineSubPR (PMSolution *t, PMSolution *s1, PMSolution *s2) {
...
double PMRelink::combinePartial (PMSolution *t, PMSolution *s1, PMSolution *s2) {
	PMInstance *inst  = t->getInstance();
	...
	best->copyFrom (s1);
	best_value = POPSTAR_INFINITY; //	best->calcCost();
	...
	for (int it=1; it<=5; it++) {
		...
		for (int i=1; i<=x; i++) {
			...
			int out;
			double profit;
			PMSearch::move (start, in, &out, &profit);
...
double PMRelink::combine_r (PMSolution *t, PMSolution *s1, PMSolution *s2, const char* method) {
...
double PMRelink::combine (PMSolution *t, PMSolution *s1, PMSolution *s2, const char* method) {
	...
	switch (method[0]) {
		...
		case 'p': result=combinePartial (t, s1, s2); break;
		case 'r': result=combine_r (t, s1, s2, method); break;
		case 'X': result=combineSubPR (t, s1, s2); break;
		case 'n': t->copyFrom(s1); result=t->calcCost(); break;
...
void PMRelink::pathRelinking (PMSolution *best_s, PMElite *elite, const char *method, bool stats, bool verbose) {
	...
	while (advance) {
		...
		if (verbose) fprintf (stderr, "GENERATION %d\n", generation);
		oldbest = best;
		local_elite->reset();
		...
		for (int i=count; i>1; i--) {
			...
			//for (int j=i+1; j<=count; j++) {
				...
				if (debug) {
					...
					if (verbose) fprintf (stderr, "Combining solutions (%d:%d); there are %d consistent solutions..\n", i, j, count);
				}
				//------------
				// get target
				//------------
				...
				if (r->computeDifference(source)>1 && r->computeDifference(target)>1) {
					PMSearch::localSearch (r, true, true);  //try to improve solution with local search
