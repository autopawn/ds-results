...
void PMSearch::runLocalSearch (PMSolution *s, char *lsmethod, bool output_density, double *profile) {
	for (int i=0; lsmethod[i]!='\0'; i++) {
		switch (lsmethod[i]) {
			...
			case 's': localSearch (s, true, true, NULL, false, output_density, profile); break;     //sparse matrix
...
void PMSearch::updateArrays (
  ...
  double *stats_total_prefix_size
	...
	for (int j=1; j<=n; j++) {
		...
		//fprintf (stderr, "%d[%f:%f:%f] ", closelist[0].id, d1, closelist[1].value, d2);
...
double PMSearch::search (PMSolution *s, bool best_improvement, PMElite *elite, bool use_list, double *density, double *times) {
    ...
    //-----------------------
	//BossaTimer tt(true);
	BossaTimer tc(false), tb(false), tu(false), tr(false), ti(true); //timers
	...
	if (times) {
		...
		*/
		times[0] += tb.getTime(); //finding best move
		times[1] += tc.getTime(); //closeness
		times[2] += ti.getTime(); //initialization
		times[3] += tr.getTime(); //rest
		times[4] += tu.getTime(); //updating structures

		//times[3] += ;

		//double rt2 = (tt.getTime() - tb.getTime() - tc.getTime() - ti.getTime() - tu.getTime());

		//fprintf (stderr, "b=%.3f c=%.3f i=%.3f r=%.3f r2=%.3f u=%.3f\n", tb.getTime(), tc.getTime(), ti.getTime(), tr.getTime(), rt2, tu.getTime());
	}
